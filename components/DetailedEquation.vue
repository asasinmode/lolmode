...
<template>
	<div :data-highlight-id="id(0)" class="flex flex-wrap gap-x-[0.28125rem] w-fit relative after:(-inset-x-1 inset-y-0 -z-1 absolute top-0 rounded-sm bg-[#c89b3c] op-20 dark:op-10)">
		<template v-for="(value, index) in content" :key="index">
			<span v-if="typeof value === 'string'">
				{{ value }}
			</span>
			<span v-else :data-id-to-highlight="index === 0 ? undefined : id(index)" class="group hover:underline relative">
